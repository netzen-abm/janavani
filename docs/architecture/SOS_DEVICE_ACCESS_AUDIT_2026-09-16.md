...
| `janavani_v2/src/main.rs` | Contains a baseline GPS coordinate in application state. | Legacy / violation | Treat as test/demo legacy material, not production location handling. Verify dependencies before archive. |
...
- [ ] Trace all active runtime imports/references to legacy SOS implementations.
- [ ] Define the provider-neutral SOS delivery/transport contract.
- [ ] Implement canonical SOS capability and tests.
- [ ] Integrate surfaces independently.
