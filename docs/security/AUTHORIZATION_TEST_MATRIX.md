...
| Agent intent without canonical authorization | DENY |
| Agent request outside data/provider scope | DENY |
| Consequential agent action without required approval | REQUIRE_APPROVAL |
...
These controls are provider-neutral so the same policy can be reused by Web, Telegram, WhatsApp, Messenger, mobile and future agent interfaces.
