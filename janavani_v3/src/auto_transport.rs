...
impl InvisibleNetworkMiddleware {
    /// Detects device connection profiles automatically to verify if a local Nym proxy loop is active [source 5].
    ...
    pub async fn auto_route_payload(endpoint_url: &str, json_payload: &str) -> Result<(ActiveTransportRoute, String), String> {
        ...
        {
            let random_delay_ms = (js_sys::Math::random() * 2000.0 + 500.0) as u64;
