...
def _request(identity: IdentityContext, context: CapabilityExecutionContext) -> ConsequentialOperationRequest:
...
def test_submission_gate_requires_consent_before_approval() -> None:
    ...
    assert gate_consequential_operation(_request(identity, context)) is ConsequentialDecision.CONSENT_REQUIRED
...
def test_submission_gate_allows_matching_consent_and_approval() -> None:
    ...
    repository.save(Consent(
        consent_id="consent-1", subject_id=identity.principal.principal_id,
        purpose="case_submission", scope=("email:government",),
        grant_type=ConsentGrantType.EXPLICIT, status=ConsentStatus.GRANTED,
        created_at="2026-09-11T00:00:00Z",
    ))
    request = _request(identity, context)
    require_execution_consent(repository, ExecutionConsentRequirement(request.consent_requirement), context)
