...
def _consent() -> Consent:
    return Consent(
        consent_id="consent-1", subject_id="citizen:submission-test", purpose=CONSENT_PURPOSE,
        scope=("email:government",), grant_type=ConsentGrantType.EXPLICIT,
        status=ConsentStatus.GRANTED, created_at="2026-09-10T00:00:00Z",
    )
...
def _prepared_case() -> tuple[CivicCaseCapability, InMemoryConsentRepository, IdentityContext, str]:
    ...
    case = case_capability.create(
        CivicCaseCreateRequest(case_type=CaseType.COMPLAINT, subject="Streetlight repair", narrative="Streetlight remains broken."),
        identity=identity,
    ).case
...
class FakeDeliveryResolver:
    def resolve(self, *, artifact_id: str, case_id: str, document_id: str) -> DeliveryArtifact:
        return DeliveryArtifact(artifact_id=artifact_id, document_id=document_id, case_id=case_id,
                                format="pdf", content=b"approved document", content_sha256="a" * 64,
...
def _ack_evidence(case_id: str) -> EvidenceObject:
    return EvidenceObject(evidence_id="evidence-ack-1", evidence_type=ACKNOWLEDGEMENT_EVIDENCE_TYPE,
                          storage_ref="evidence://ack-1", sha256="b" * 64,
                          received_at="2026-09-10T00:00:00Z", source_description="Destination acknowledgement evidence",
                          status="ACTIVE")
...
def test_submission_failure_persists_failed_delivery_without_claiming_success() -> None:
    ...
    capability = SubmissionCapability(cases, consents, FakeTransport(error=RuntimeError("transport down")), submissions)
...
def test_delivery_transport_requires_approved_artifact() -> None:
    ...
    capability = SubmissionCapability(cases, consents, submission_repository=submissions,
...
def test_delivery_submission_requires_independent_acknowledgement_evidence() -> None:
    ...
    transport = FakeDeliveryTransport(DeliveryReceipt(external_reference="ext-123", transport_reference="transport-123",
                                                       acknowledgement_evidence_ref=evidence_obj.evidence_id,
                                                       notes="Accepted by destination"))
    capability = SubmissionCapability(cases, consents, submission_repository=submissions,
                                       delivery_transport=transport, artifact_resolver=FakeDeliveryResolver(),
                                       evidence_repository=evidence)
    result = capability.submit(_request(case_id, artifact_id="artifact-1"), identity=identity, explicit_user_approval=True)
...
def test_delivery_without_acknowledgement_evidence_stays_submitted() -> None:
    ...
    capability = SubmissionCapability(cases, consents, submission_repository=submissions,
                                       delivery_transport=transport, artifact_resolver=FakeDeliveryResolver(),
                                       evidence_repository=evidence)
    result = capability.submit(_request(case_id, artifact_id="artifact-2"), identity=identity, explicit_user_approval=True)
...
def test_acknowledge_with_evidence_requires_attached_evidence() -> None:
    ...
    submissions = InMemorySubmissionRepository()
    evidence = InMemoryEvidenceRepository()
    capability = SubmissionCapability(cases, consents, FakeTransport(), submissions, evidence_repository=evidence)
    capability.submit(_request(case_id), identity=identity, explicit_user_approval=True)
    evidence.save(_ack_evidence(case_id))
    with pytest.raises(PermissionError, match="not attached"):
        capability.acknowledge_with_evidence(submissions.list_for_case(case_id)[0].submission_id,
...
def test_execution_envelope_is_validated_for_submission() -> None:
    cases, consents, identity, case_id = _prepared_case()
    capability = SubmissionCapability(cases, consents, FakeTransport(), InMemorySubmissionRepository())
    context = CapabilityExecutionContext.for_capability(identity, capability_id=SUBMIT_CAPABILITY,
                                                        action="case:submit", surface="telegram",
                                                        resource_id=case_id, side_effect_class="external_side_effect",
                                                        idempotency_key="submission-test-1")
    result = capability.submit(_request(case_id), identity=identity, explicit_user_approval=True,
                               execution_context=context)
    assert result.case.status is CaseStatus.SUBMITTED


def test_execution_envelope_rejects_wrong_resource() -> None:
    cases, consents, identity, case_id = _prepared_case()
    capability = SubmissionCapability(cases, consents, FakeTransport(), InMemorySubmissionRepository())
    context = CapabilityExecutionContext.for_capability(identity, capability_id=SUBMIT_CAPABILITY,
                                                        action="case:submit", surface="webapp",
                                                        resource_id="case:other", side_effect_class="external_side_effect",
                                                        idempotency_key="submission-test-2")
