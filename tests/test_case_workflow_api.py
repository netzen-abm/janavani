...
def test_create_and_get_case() -> None:
    ...
    response = client.post("/cases", json={"issue": "Broken streetlight"})
    assert response.status_code == 201
    case = response.json()
    assert case["issue"] == "Broken streetlight"

    fetched = client.get(f"/cases/{case['case_id']}")
...
def test_submission_requires_workflow_prerequisites() -> None:
    client = TestClient(create_canonical_app())
    created = client.post("/cases", json={"issue": "Service delay"}).json()
    response = client.post(
        f"/cases/{created['case_id']}/submission",
...
def test_submission_cannot_be_read_under_another_case() -> None:
    client = TestClient(create_canonical_app())
    first = client.post("/cases", json={"issue": "First"}).json()
    second = client.post("/cases", json={"issue": "Second"}).json()
    response = client.get(
        f"/cases/{second['case_id']}/submission/not-owned-by-case"
    )
    assert response.status_code == 404
