...
from src.storage.serialization import authority_row, case_row, consent_row, document_row, evidence_row, submission_row
...
def test_evidence_row_preserves_metadata_and_provenance() -> None:
    evidence = Evidence.create("CASE-1", EvidenceKind.DOCUMENT, "Order", "citizen", content_ref="blob://1", provenance=["official-ref"])
...
def test_authority_row_serializes_sources() -> None:
    source = AuthoritySource(source_id="SRC-1", source_type="official_web", uri="https://example.gov", publisher="Example Government")
...
def test_consent_row_serializes_time_bounds() -> None:
    ...
    consent = Consent(consent_id="CON-1", subject_id="CASE-1", capability_id="CAP-SUBMIT", purpose="Submit representation", scope=("case",), data_categories=("personal",), status=ConsentStatus.GRANTED, policy_version="v1", source_channel="web", granted_at=granted)
...
def test_document_row_matches_canonical_document_fields() -> None:
    document = Document.create(DocumentType.COMPLAINT, "Complaint", "en", PartyRef("office", "Example Office"), "Delayed service", "Please investigate.", case_id="CASE-1")
...
def test_submission_row_preserves_idempotency_and_references() -> None:
    submission = Submission(case_id="CASE-1", destination_ref="AUTH-1", consent_ref="CON-1", authorization_ref="AUTHZ-1", payload_hash="sha256:x")
