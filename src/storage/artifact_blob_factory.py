...
SUPPORTED_ARTIFACT_BLOB_PROVIDERS = {"local"}


def create_artifact_blob_store(
    provider: str | None = None,
) -> ArtifactBlobStore:
    """Create an artifact blob provider without coupling capabilities to it."""
    selected = (
        provider
    ...
    if selected == "local":
        root = os.getenv("JANAVANI_ARTIFACT_BLOB_ROOT", "/tmp/janavani-artifacts")
