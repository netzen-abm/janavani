...
async def add_evidence(case_id: str, request: EvidenceRequest) -> dict[str, object]:
    case = _get_case(case_id)
    event = case.add_evidence(request.evidence_id, event_id=request.event_id,
                              occurred_at=request.occurred_at, actor_id=request.actor_id,
...
async def mark_ready(case_id: str, request: EventRequest) -> dict[str, object]:
    case = _get_case(case_id)
    event = case.mark_ready(event_id=request.event_id, occurred_at=request.occurred_at,
...
async def submit_case(case_id: str, request: EventRequest) -> dict[str, object]:
    case = _get_case(case_id)
    event = case.submit(event_id=request.event_id, occurred_at=request.occurred_at,
                        actor_id=request.actor_id, source_channel=request.source_channel)
    return {"case_id": case.case_id, "status": case.status.value, "event": event.event_type.value}
