...
Persistence is not part of this first contract extraction.
"""
...
async def mark_ready(case_id: str, request: EventRequest) -> dict[str, object]:
    event = _get_case(case_id).mark_ready(event_id=request.event_id, occurred_at=request.occurred_at, actor_id=request.actor_id)
    return {"case_id": case_id, "status": _get_case(case_id).status.value, "event": event.event_type.value}


@router.post("/{case_id}/submit")
async def submit_case(case_id: str, request: EventRequest) -> dict[str, object]:
    event = _get_case(case_id).submit(event_id=request.event_id, occurred_at=request.occurred_at, actor_id=request.actor_id, source_channel=request.source_channel)
    return {"case_id": case_id, "status": _get_case(case_id).status.value, "event": event.event_type.value}
...
def _serialize(case: CivicCase) -> dict[str, object]:
    return {
        ...
        "events": [
            {"event_id": e.event_id, "event_type": e.event_type.value, "occurred_at": e.occurred_at, "actor_id": e.actor_id, "source_channel": e.source_channel, "notes": e.notes}
