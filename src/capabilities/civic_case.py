...
This module is deliberately small: it composes the canonical domain model,
identity/authorization and persistence contracts. Web, Telegram and future
surfaces should invoke this capability instead of implementing case creation
independently.
"""
...
class CivicCaseCapability:
    ...
    def create(
        ...
        *,
        ...
        case.events.append(
            # Creation is the initial event; the domain model remains the owner
            # of subsequent lifecycle transitions.
            __import__("src.core.civic_case", fromlist=["CaseEvent"]).CaseEvent(
                event_id=f"event-{uuid4().hex}",
                case_id=case_id,
                event_type=__import__("src.core.civic_case", fromlist=["CaseEventType"]).CaseEventType.CREATED,
                occurred_at=now,
