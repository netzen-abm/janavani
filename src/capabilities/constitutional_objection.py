...
class ConstitutionalObjectionCapability:
    ...
    def build_document(self, case_id: str, *, identity: Any, bill_code: str,
                       citizen_comments: str, document_id: str | None = None) -> ConstitutionalObjectionBuildResult:
        ...
        if self._evidence_repository is not None:
            missing = [eid for eid in case.evidence_refs if self._evidence_repository.get(eid) is None]
            if missing:
        ...
        draft = DocumentDraft(
            ...
            body=body,
            to=DocumentParty(name=destination.name, address=destination.address,
                             email=destination.email, role=destination.role),
            cc=tuple(DocumentParty(name=c.name, address=c.address, email=c.email, role=c.role)
                     for c in authority.cc_contacts),
        )
        return ConstitutionalObjectionBuildResult(case.case_id, authority_id, bill_code, draft)

    def generate_reviewable_artifact(self, case_id: str, *, identity: Any, bill_code: str,
        ...
        if case is not None and artifact.reference.artifact_id not in case.document_refs:
            case.add_document(artifact.reference.artifact_id,
                              event_id=f"{case.case_id}:document:{document_format.value}",
                              occurred_at=datetime.now(timezone.utc).isoformat(), source_channel=None)
    ...
    def _compose_body(bill_data: dict[str, Any], evaluation: dict[str, Any], citizen_comments: str) -> str:
        state, title = bill_data["state"], bill_data["title"]
        return (
