    ...
    def record_submission_consent(
        ...
        *,
        ...
        if consent is None:
            ...
            consent.subject_id != identity.principal.principal_id
            or not consent.authorizes(PURPOSE_SUBMISSION, scope)
        ):
            raise PermissionError("Existing consent does not authorize this identity and scope")

        attached = self._cases.add_consent(
            case_id,
            consent.consent_id,
            identity=identity,
        )
