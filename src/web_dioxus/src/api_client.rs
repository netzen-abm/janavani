...
impl JanavaniDioxusBridge {
    pub fn new() -> Self {
        ...
        let is_freenet =
            current_url.contains("freenet") || current_url.contains("127.0.0.1:5050");
    ...
    pub async fn dispatch_draft_workflow(
        &self,
        input_text: String,
        ...
        if self.is_freenet_context {
            return Ok(DraftResponse {
                ...
                document: DocumentPayload {
                    ...
                    suggested_ministry_or_department:
                        "Competent Local Self Government Department".to_string(),
                    factual_points: vec![input_text],
                    legal_or_policy_basis: vec!["Section 6(1) of the RTI Act, 2005".to_string()],
                    specific_prayers_or_requests:
                        vec!["Immediate administrative resolution requested.".to_string()],
                },
            });
        }

        let backend_url = self.backend_url.as_deref().ok_or_else(|| {
            "No conventional backend is configured; local/decentralized capabilities remain available."
                .to_string()
        })?;
