"""Channel-neutral authority resolution contract for civic cases."""
...
def resolve_authority(case_id: str, candidates: list[AuthorityCandidate]) -> AuthorityResolution:
    """Return candidates without silently selecting an unverified authority."""
    if not case_id:
        raise ValueError("case_id is required")
    if not candidates:
        return AuthorityResolution(case_id=case_id, candidates=())
    ...
    return AuthorityResolution(case_id=case_id, candidates=tuple(candidates), selected_office_id=selected)
